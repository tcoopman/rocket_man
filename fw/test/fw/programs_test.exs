defmodule Fw.ProgramsTest do
  ...
  describe "Walker tests" do
    ...
    test "1 on" do
      ...
      expected =
        <<0, 0, 0, 0>> <>
          <<255, 255, 0, 0, 255, 0, 0, 0, 255, 0, 0, 0, 255, 255, 0, 0>> <> close()
    ...
    test "all on" do
      ...
      expected =
        <<0, 0, 0, 0>> <>
          <<255, 255, 0, 0, 255, 255, 0, 0, 255, 255, 0, 0, 255, 255, 0, 0>> <> close()
  ...
  describe "WalkerDown test" do
    test "start" do
      state = WalkerDown.new(10)
      {command, sleep, _state} = WalkerDown.execute(state)
    ...
    test "up then down" do
      state = WalkerDown.new(4)
      {_command, _sleep, state} = WalkerDown.execute(state)
      assert :up = state.up_or_down
      assert 1 = state.iteration
      {_command, _sleep, state} = WalkerDown.execute(state)
      assert :up = state.up_or_down
      assert 2 = state.iteration
      {_command, _sleep, state} = WalkerDown.execute(state)
      assert :down = state.up_or_down
      assert 1 = state.iteration
      {_command, _sleep, state} = WalkerDown.execute(state)
      assert :down = state.up_or_down
      assert 0 = state.iteration
